...
samples=100 #number of data to be sent to cloud in one cycle
counter100 = 0
...
headers="GFORCEX,GFORCEY,GFORCEZ,ROT_X,ROT_Y,ROT_Z,TEMP C,TEMP F,GSR,HEARTRATE,SpO2 %,AMB TEMP MLX, OBJ TEMP MLX"  #Added TEMP MLX for the MLX90614
...
while True:
    while batchesof100:
        ...
        print("writing data to file " + "temp.csv")
        while( line <= samples ):
            bus = SMBus(1)
            tempSensor = MLX90614(bus, address=0x5A)
            print("batchesof100 = " + str(batchesof100) + " Line = " + str(line) + ": writing...")
    ...
    mainFile = open("all.csv", "w" )

    for x in range(999):
        mainFile.iloc[1,:]

    with open('temp.csv', 'r') as f1:
        tempo = f1.read()

    mainFile.write('\n')
    mainFile.write(tempo)
