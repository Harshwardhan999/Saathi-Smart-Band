...
while(1):
    ...
    file = open("servermini-v3.csv", "a" )
    print("opened file")

    df = pd.read_csv("servermini-v3.csv")
    line=0
    df=df.drop(2,axis=0,inplace=True)
    df.to_csv("servermini-v2.csv", index=False)
    bus = SMBus(1)
    ...
    # print("writing data to file " + "servermini-v2.csv")
    # while( line <= samples ):
    #     bus = SMBus(1)
    #     tempSensor = MLX90614(bus, address=0x5A)
    #     if line==0:
